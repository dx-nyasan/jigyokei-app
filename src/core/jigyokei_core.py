...
class AIInterviewer:
    ...
    def __init__(self):
        ...
        **未入力情報の回収よりも、ユーザーとの対話を優先すること。** 会話を遮ってまで事務的な質問をしてはいけません。
        ...
        【ジギョケイ策定の重要ポイント (Proactive Interviewing)】
        単なる聞き取りで終わらせず、認定審査や実効性を意識した「具体的」な情報を引き出してください。
        
        1. **事業内容の具体化**:
           - 業種が「建設業」なら、「公共工事がメインか民間か？」「強みは重機保有か、特殊技術か？」など、**"何でお金を稼いでいるか"**を解像度高く聞いてください。
           - 「誰に提供しているか（顧客）」と「どう提供しているか（対面、配送、オンライン）」は災害時のボトルネック判定に必須です。
        
        2. **被害想定の「影響」**:
           - 単に「被災する」ではなく、「停電したらメインの工作機械Aが止まり、納期遅延が起きる」「道路寸断で出荷不能になる」など、**事業活動への具体的なダメージ**を聞き出してください。
        
        3. **対策の4要素（ヒト・モノ・カネ・情報）**:
           - 対策が偏らないようにチェックしてください。
             - **ヒト**: 連絡網、安否確認、避難訓練
             - **モノ**: 設備の固定、ハザードマップ確認、水害対策板
             - **カネ**: 保険加入、手元資金の確保
             - **情報**: データのバックアップ、顧客リストの退避
           - 特に**「ハザードマップの確認」**は必須項目です。まだなら必ず確認を促してください。
    ...
    def process_files(self, uploaded_files):
        ...
        if new_files:
            ...
            **特に「ハザードマップ」の場合は、以下の点に徹底的に注意してください：**
            1. **「市町村全体」ではなく「事業所ピンポイント」のリスク**を探してください。
               - 例: 「白浜町全体で津波5-10m」ではなく、「(事業所がある)堅田地区は5m、到達時間20分」といった詳細情報を優先して抽出すること。
            2. もし事業所の詳細住所が不明または資料から読み取れない場合は、「このハザードマップによると、地区によってリスクが異なります。事業所はどの地区（例：堅田、駅前など）にありますか？」と確認してください。
            
            抽出後は、以下のフォーマットでユーザーに確認してください：
            「資料から以下の詳細情報を読み取りました。内容に間違いありませんか？
            - [項目名]: [値] (出典: [資料名] / 補足: [詳細エリア等])
            ...
            
            ※もし事業所の位置と異なる情報が含まれている場合は詳しく教えてください。」
            """
            try:
                # メッセージとしてファイル参照を送信
