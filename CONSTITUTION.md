...
|:---|:---:|:---|
| **経営支援の理念** | | |
| 寄り添い（3ロール対応） | 95% | USER_MANUAL 3種、ロール別UI完備 |
| 引き出し（AI対話） | 90% | クイック返信、スマート抽出実装 |
| 導く（改善提案） | 80% | 監査エージェント稼働、RAG統合済 |
| **アーキテクチャ理念** | | |
| 認定駆動 | 90% | certification_requirements.json実装 |
| 透明性 | 85% | スコア内訳表示、X/Y形式 |
| 持続性 | 75% | RAG稼働だがチャンク品質に課題 |
