...
## 手順
1. **プロトコル遵守の確認- **手順**:
  1. `core-protocol` を参照し、プロジェクト理念（経営支援・建築・UX）に反していないか確認。
  2. `jigyokei-domain-expert` を参照し、審査基準のバリデーションに漏れがないか確認。
  3. `docs/USER_JOURNEY.md` を参照し、新しいコードが過去のUXステップを複雑化させたり、利便性を損なわせたりしていないか（UX退行）をチェック。
  4. GEMINI.mdのTDDサイクルが守られているか、テスト実行ログから検証。
3. **GEMINI.mdの参照**: 常に `GEMINI.md` を読み込み、現在のルールセットを確認する。
2. **静的解析**:
